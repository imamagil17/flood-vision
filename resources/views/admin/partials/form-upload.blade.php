<section class="lg:col-span-5 bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm">
    ...
    <form id="standaloneVideoForm" class="space-y-5" onsubmit="executeYoloSimulation(event)">
        ...
        <div id="dropzone" class="border-2 border-dashed border-slate-300 hover:border-blue-500 bg-slate-50/50 rounded-2xl p-6 transition-all duration-300 text-center relative cursor-pointer group">
            <input type="file" id="videoFileInput" accept=".mp4,.mov,.avi" class="absolute inset-0 opacity-0 cursor-pointer z-10" required>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <input type="date" id="input_tanggal" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" required>
                <input type="time" id="input_waktu" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" required>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea id="keterangan" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-all" placeholder="Tambahkan catatan kondisi sungai..."></textarea>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-xl shadow-md shadow-blue-500/10 hover:shadow-lg transition-all text-sm flex items-center justify-center gap-2">
